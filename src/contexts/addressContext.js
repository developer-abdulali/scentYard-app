...
//   initialState,
//   initialUserObj,
// } from "../constants/reducers/addressReducer";
...
const AddressProvider = ({ children }) => {
  ...
  useEffect(() => {
    try {
      (async () => {
        const { data, status } = await getAddressService();
  ...
  const submitFormHandler = async (e) => {
    ...
    if (addressExists) {
      ...
      try {
        const { data, status } = await addAddressService(addressState.formData);
  ...
  const deleteAddressHandler = async (addressId) => {
    try {
      const { data, status } = await deleteAddressService(addressId);
